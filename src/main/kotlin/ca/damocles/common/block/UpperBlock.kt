...
class UpperBlock(settings: Settings?) : BlockWithEntity(settings){
    ...
    init{
        defaultState = ((stateManager.defaultState as BlockState).with(FACING, Direction.UP) as BlockState).with(ENABLED, true) as BlockState
    }
    ...
    override fun getRaycastShape(state: BlockState, world: BlockView?, pos: BlockPos?): VoxelShape? {
        return when (state.get(FACING) as Direction) {
            Direction.UP -> UP_RAY_TRACE_SHAPE
    ...
    override fun getPlacementState(ctx: ItemPlacementContext): BlockState? {
        ...
        return (defaultState.with(FACING, if (direction.axis === Direction.Axis.Y) Direction.UP else direction) as BlockState).with(ENABLED, true) as BlockState
    }

    override fun onPlaced(world: World, pos: BlockPos?, state: BlockState?, placer: LivingEntity?, itemStack: ItemStack) {
        if (itemStack.hasCustomName()) {
            val blockEntity = world.getBlockEntity(pos)
            if (blockEntity is UpperBlockEntity) {
